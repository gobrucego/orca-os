...
**Goal:** Use SAME.dev’s cloning playbook (via web analysis) to produce a high‑fidelity UI/structure spec for a target site, then hand off to OS 2.0 pipelines (`/orca-nextjs` / `/orca-expo` lanes) for implementation. This command focuses on **spec and structure**, not blindly autogenerated code.
...
- Actual implementation will go through the appropriate OS 2.0 lane pipelines with quality gates (design-dna/tokens, a11y, performance, security, verification), not as a one-off clone.
