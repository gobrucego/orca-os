...
- Inputs:
  - Components/pages to test, routes, and acceptance criteria
- Outputs:
  - Test files under `src/**/__tests__/` or co-located `*.test.ts[x]`
  - Logs in `.orchestration/logs/`, evidence (screenshots) in `.orchestration/evidence/` if visual diffs
  - Update `.orchestration/implementation-log.md` with `#FILE_CREATED` and `#FILE_MODIFIED`
- Acceptance:
  - Tests pass locally and under `/finalize`
  - Coverage or critical path assertions met (as agreed)
- Self-checks:
  - `rg -n "describe\(|it\(" src`
  - `rg -n "#FILE_CREATED|#FILE_MODIFIED" .orchestration/implementation-log.md`
