...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            echo "Usage: $0 [--dry-run] [--target <path>]"; exit 0 ;;
        *)
...
if [ ! -d "$DEPLOY_TARGET" ]; then
    if [ "$DRY_RUN" = false ]; then
        ...
        print_success "Created deployment target: $DEPLOY_TARGET"
    else
        print_warning "Target missing; would create: $DEPLOY_TARGET"
    fi
fi
...
if [ -f "$REPO_ROOT/commands/session-save.md" ]; then REMOVED_DEPRECATED=$((REMOVED_DEPRECATED+1)); fi
if [ -f "$REPO_ROOT/commands/session-resume.md" ]; then REMOVED_DEPRECATED=$((REMOVED_DEPRECATED+1)); fi
...
# Remove deprecated commands even if present in source (legacy cleanup)
if [ "$DRY_RUN" = false ]; then
    delete_file "commands/session-save.md"
    delete_file "commands/session-resume.md"
fi
...
if [ -f "$REPO_ROOT/.claude/DEPLOYMENT_MANIFEST.md" ]; then
...
if [ "$DRY_RUN" = false ]; then
    ...
    EXPECTED_COMMANDS=$((COMMAND_COUNT-REMOVED_DEPRECATED))
    if [ "$DEPLOYED_COMMANDS" -eq "$EXPECTED_COMMANDS" ]; then
        print_success "Commands: $DEPLOYED_COMMANDS (expected $EXPECTED_COMMANDS)"
    else
        print_error "Commands: $DEPLOYED_COMMANDS (expected $EXPECTED_COMMANDS) - MISMATCH!"
    fi
