...
- Inputs:
  - `.orchestration/implementation-log.md` with tags (#FILE_CREATED, #FILE_MODIFIED, #SCREENSHOT_CLAIMED, #PATH_DECISION)
  - Project workspace files to verify claims
- Outputs:
  - Verification report under `.orchestration/verification/` and updated `.verified` via `/finalize`
- Acceptance:
  - All referenced files exist; screenshots valid when claimed; counts/grep checks pass
  - `/finalize` reports Zero-Tag Gate PASS
- Self-checks:
  - `ls .orchestration/implementation-log.md`
  - `rg -n "#FILE_CREATED|#FILE_MODIFIED|#SCREENSHOT_CLAIMED|#PATH_DECISION" .orchestration/implementation-log.md`
---
