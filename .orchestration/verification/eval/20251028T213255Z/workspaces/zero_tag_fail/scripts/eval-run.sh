...
run_finalize() {
  local ws="$1"
  local t0=$(date +%s)
  (cd "$ws" && bash scripts/finalize.sh) >/dev/null 2>&1 || true
  local rc=$?
  local t1=$(date +%s)
  local dur=$((t1 - t0))
  echo "$rc $dur"
...
parse_field() {
  ...
  sed -n "s/^- ${key}: \(.*\)$/\1/p" "$ws/.orchestration/verification/verification-report.md" | head -1
}
...
main() {
  ...
  mkdir -p "$ws/out" && echo '<html>ok</html>' > "$ws/out/index.html"
  # ensure no implementation log
  rm -f "$ws/.orchestration/implementation-log.md" || true
  read rc dur < <(run_finalize "$ws")
